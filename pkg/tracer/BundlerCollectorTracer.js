...
var tracer = {
  ...
  result: function result(_ctx, _db) {
    return {
      callsFromAiMiddleware: this.callsFromAiMiddleware,
      keccak: this.keccak,
      logs: this.logs,
      calls: this.calls,
  ...
  step: function step(log, db) {
    ...
    if (log.getDepth() === 1) {
      if (opcode === "CALL" || opcode === "STATICCALL") {
        ...
        this.currentLevel = this.callsFromAiMiddleware[this.topLevelCallCounter] =
          {
    ...
    if (opcode === "SLOAD" || opcode === "SSTORE") {
      ...
      if (opcode === "SLOAD") {
        // read slot values before this AiOp was created
